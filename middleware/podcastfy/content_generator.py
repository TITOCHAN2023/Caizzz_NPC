...
class LongFormContentGenerator:
    ...
    def generate_long_form(
        ...
        logger.info(f"Generating {num_parts} parts")
        ...
        for i, chunk in enumerate(chunks):
            ...
            logger.info(f"Generated part {i+1}/{num_parts}: Size {len(chunk)} characters.")
            #logger.info(f"[LLM-START] Step: {i+1} ##############################")
            #logger.info(response)
            #logger.info(f"[LLM-END] Step: {i+1} ##############################")
    ...
    def generate_qa_content(
        ...
        try:
            ...
            if output_filepath:
                ...
                logger.info(f"Transcript saved to {output_filepath}")
