...
def get_config_path(config_file: str = 'config.yaml'):
	...
	try:
		...
		logger.info(f"Error locating {config_file}: {str(e)}")
		return None
...
class Config:
	def __init__(self, config_file: str = 'config.yaml'):
		...
		if dotenv_path:
			...
			logger.info("Warning: .env file not found. Using environment variables if available.")
		...
		if config_path:
			...
			logger.info("Could not locate config.yaml")
			self.config = {}
...
def main() -> None:
	...
	logger.info(f"JINA_API_KEY: {'Set' if config.JINA_API_KEY else 'Not set'}")
	logger.info(f"GEMINI_API_KEY: {'Set' if config.GEMINI_API_KEY else 'Not set'}")
	logger.info(f"OPENAI_API_KEY: {'Set' if config.OPENAI_API_KEY else 'Not set'}")
	logger.info(f"ELEVENLABS_API_KEY: {'Set' if config.ELEVENLABS_API_KEY else 'Not set'}")
	...
	logger.info(f"NON_EXISTENT_KEY: {config.get('NON_EXISTENT_KEY', 'Default Value')}")
